...
<section class="card" style="margin-top:14px"><h2>מסד הנתונים</h2><div class="hint">צפייה, עריכה ומחיקה של רשומות. סיסמאות ומסוף SQL אינם חשופים בדף.</div><div class="actions"><select id="db-table"><option value="requests">פניות והובלות</option><option value="contacts">אנשי קשר</option><option value="conversations">שיחות</option><option value="messages">הודעות נכנסות</option><option value="outbox">הודעות יוצאות</option></select><button id="db-load" type="button">טען רשומות</button></div><div class="table"><table><thead id="db-head"></thead><tbody id="db-rows"><tr><td>בחר טבלה וטען נתונים</td></tr></tbody></table></div><hr><h2>ניקוי סביבת בדיקות</h2><div class="hint">מוחק לצמיתות את כל הנתונים התפעוליים: פניות, הודעות, שיחות, אנשי קשר, קבצים מקושרים, תורים ואירועים. הגדרות מערכת ורשימות יישובים נשארות.</div><div class="actions"><button id="db-clear-all" class="danger" type="button">מחק את כל הרשומות</button></div></section></main>
<script>
  ...
  await app.register(
    async (admin) => {
      ...
      admin.get("/database", async (req) => {
        ...
        const views = {
          requests: {
            columns: ["number","status","donor_phone","donor_name","pickup_city","pickup_address","pickup_floor","receiver_phone","receiver_name","destination_city","destination_address","destination_floor","items","quantity","needs_disassembly","requested_date","run_date","human_reason","donor_approved","receiver_approved","photos","created_at","updated_at"],
            editableFields: ["status", "run_date", "human_reason"],
            sql: `SELECT r.id,r.number,r.status,dc.phone AS donor_phone,d.name AS donor_name,d.settlement AS pickup_city,d.address AS pickup_address,d.floor AS pickup_floor,rc.phone AS receiver_phone,v.name AS receiver_name,v.settlement AS destination_city,v.address AS destination_address,v.floor AS destination_floor,string_agg(i.description, ', ' ORDER BY i.position) AS items,coalesce(sum(i.quantity),0)::int AS quantity,bool_or(i.needs_disassembly) AS needs_disassembly,r.earliest_run_date AS requested_date,r.run_date,r.human_reason,d.approved_at IS NOT NULL AS donor_approved,v.approved_at IS NOT NULL AS receiver_approved,(SELECT count(*)::int FROM request_media rm WHERE rm.request_id=r.id) AS photos,r.created_at,r.updated_at FROM requests r LEFT JOIN request_parties d ON d.request_id=r.id AND d.role='donor' LEFT JOIN contacts dc ON dc.id=d.contact_id LEFT JOIN request_parties v ON v.request_id=r.id AND v.role='receiver' LEFT JOIN contacts rc ON rc.id=v.contact_id LEFT JOIN request_items i ON i.request_id=r.id GROUP BY r.id,dc.phone,d.name,d.settlement,d.address,d.floor,d.approved_at,rc.phone,v.name,v.settlement,v.address,v.floor,v.approved_at ORDER BY r.number DESC LIMIT $1`,
          },
          contacts: {
