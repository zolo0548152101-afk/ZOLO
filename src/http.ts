...
<section class="card" style="margin-top:14px"><h2>מסד הנתונים</h2><div class="hint">צפייה, עריכה ומחיקה של רשומות. סיסמאות ומסוף SQL אינם חשופים בדף.</div><div class="actions"><select id="db-table"><option value="requests">פניות והובלות</option><option value="contacts">אנשי קשר</option><option value="conversations">שיחות</option><option value="messages">הודעות נכנסות</option><option value="outbox">הודעות יוצאות</option></select><button id="db-load" type="button">טען רשומות</button></div><div class="table"><table><thead id="db-head"></thead><tbody id="db-rows"><tr><td>בחר טבלה וטען נתונים</td></tr></tbody></table></div>${c.BOT_MODE === "live" ? "" : '<hr><h2>ניקוי סביבת בדיקות</h2><div class="hint">מוחק לצמיתות את כל הנתונים התפעוליים: פניות, הודעות, שיחות, אנשי קשר, קבצים מקושרים, תורים ואירועים. הגדרות מערכת ורשימות יישובים נשארות.</div><div class="actions"><button id="db-clear-all" class="danger" type="button">מחק את כל הרשומות</button></div>'}</section></main>
<script>
